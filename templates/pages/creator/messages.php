...
$mobileConversationListUrl = path_with_query('/creator/messages', ['q' => $filters['q'] ?? '']);
?>
<!DOCTYPE html>
...
<main class="min-h-screen px-6 pb-10 pt-24 lg:ml-64 lg:px-10">
    <section class="mb-8">
        ...
        <p class="mt-4 max-w-2xl text-on-surface-variant">Converse com sua base pagante, envie anexos, conteudos privados e microconteudos sem sair da conversa.</p>
    </section>
    ...
    <div class="grid grid-cols-1 gap-8 xl:grid-cols-[0.92fr_1.08fr]">
        <section class="space-y-6">
            ...
            <div class="space-y-4">
                <?php foreach ($conversations as $conversation): ?>
                    ...
                    <a class="block rounded-3xl p-5 shadow-sm transition-colors <?= $active ? 'bg-primary text-white' : 'bg-surface-container-lowest hover:bg-surface-container-low' ?>" data-chat-conversation-card="<?= e((string) ($conversation['id'] ?? 0)) ?>" href="<?= e('/creator/messages?conversation=' . (int) ($conversation['id'] ?? 0)) ?>">
        ...
        <section class="overflow-hidden rounded-3xl bg-surface-container-lowest p-0 shadow-sm lg:p-8" data-mobile-chat-panel data-mobile-chat-state="<?= ($selectedAnnouncement || $selectedConversation) ? 'selected' : 'empty' ?>">
                ...
                <div class="flex h-full min-h-0 flex-col px-4 lg:h-[78vh] lg:px-0">
                    ...
                    <div class="flex items-center gap-4 border-b border-slate-200 pb-4">
                        ...
                        <?php if ((string) (($selectedConversation['subscription']['plan']['name'] ?? '')) !== ''): ?>
                            <span class="hidden rounded-full bg-primary/10 px-3 py-2 text-[10px] font-bold uppercase tracking-[0.2em] text-primary sm:inline-flex"><?= e((string) ($selectedConversation['subscription']['plan']['name'] ?? 'Plano ativo')) ?></span>
                        <?php endif; ?>
                    </div>
                                                                ...
                                                                <div class="min-w-0 flex-1">
                                                                    ...
                                                                    <div class="mt-3 flex flex-wrap gap-2">
                                                                        <?php if ((string) ($message['required_plan_name'] ?? '') !== ''): ?>
                                                                            <span class="inline-flex rounded-full bg-primary/10 px-3 py-2 text-xs font-bold text-primary"><?= e((string) ($message['required_plan_name'] ?? 'Plano')) ?></span>
                                                                        <?php endif; ?>
                                                                        <?php if ((int) ($message['unlock_price'] ?? 0) > 0): ?>
                                                                            <span class="inline-flex rounded-full bg-primary/10 px-3 py-2 text-xs font-bold text-primary"><?= luacoin_amount_html((int) ($message['unlock_price'] ?? 0), 'inline-flex items-center gap-1.5 whitespace-nowrap', '', 'h-3.5 w-3.5 shrink-0') ?></span>
                                                                        <?php endif; ?>
                    ...
                    <form action="/creator/messages/send" class="mt-3 shrink-0 border-t border-slate-200 pt-3" data-chat-compose data-chat-role="creator" enctype="multipart/form-data" method="post">
                        ...
                        <div class="mt-3 flex items-end gap-3 pb-[calc(env(safe-area-inset-bottom,0px)+0.5rem)] lg:pb-0">
                            ...
                            <button class="flex h-12 w-12 items-center justify-center rounded-full bg-primary text-white shadow-lg shadow-primary/20" data-chat-submit type="submit">
            ...
            <?php else: ?>
                <div class="flex h-full min-h-[420px] items-center justify-center rounded-none bg-surface-container-low p-8 text-center text-on-surface-variant lg:rounded-3xl">
