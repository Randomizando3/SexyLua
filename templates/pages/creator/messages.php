        ...
        <section class="rounded-3xl bg-surface-container-lowest p-8 shadow-sm">
            ...
            <?php elseif ($selectedConversation): ?>
                <div class="flex min-h-[72vh] flex-col">
                    ...
                    <form action="/creator/messages/send" class="mt-4 shrink-0 rounded-3xl bg-surface-container-low p-4 shadow-sm" enctype="multipart/form-data" method="post">
                        <input name="_token" type="hidden" value="<?= e($app->csrf->token()) ?>">
                        <input name="conversation_id" type="hidden" value="<?= e((string) ($selectedConversation['id'] ?? 0)) ?>">
                        <div class="flex items-end gap-3">
                            <textarea class="h-16 flex-1 resize-none rounded-2xl border-none bg-white px-4 py-3 text-sm shadow-sm focus:ring-2 focus:ring-primary/20" name="body" placeholder="Mensagem, descricao do anexo ou descricao do microconteudo..."></textarea>
                            <button class="rounded-full bg-primary px-5 py-3 text-sm font-bold text-white shadow-lg shadow-primary/20" type="submit">Enviar</button>
                        </div>
