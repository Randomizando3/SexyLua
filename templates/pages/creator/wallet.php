...
$canWithdraw = (bool) ($data['can_withdraw'] ?? false);
$typeLabels = [
...
$creatorTopbarAction = ['href' => '/creator/live', 'label' => 'Live'];
...
<main class="px-6 pb-12 pt-24 lg:ml-64 lg:px-10">
    <div class="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="mt-3 max-w-3xl text-slate-500">Saldo em LuaCoins, receita por assinatura, gorjetas e solicitacoes de saque com chave PIX.</p>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6 xl:grid-cols-2">
        <section class="signature-glow flex h-full flex-col justify-between rounded-3xl p-8 text-white shadow-[0px_20px_40px_rgba(171,17,85,0.18)]">
            ...
            <div class="mt-6 grid grid-cols-2 gap-4">
                <div class="rounded-2xl bg-white/12 p-4">
                    <p class="text-[10px] font-bold uppercase tracking-[0.22em] text-white/65">Receita liquida</p>
        ...
        <form action="/creator/wallet/payout" class="flex h-full flex-col rounded-3xl bg-white p-8 shadow-[0px_20px_40px_rgba(27,28,29,0.06)]" method="post">
            ...
            <div>
                ...
                <p class="mt-2 text-sm text-slate-500">Defina o valor e a chave PIX de recebimento. O historico completo fica logo abaixo.</p>
            </div>
            <?php if ($verificationStatus !== 'approved'): ?>
                <div class="mt-6 rounded-3xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm font-semibold text-amber-900">
                    Seu perfil ainda nao foi verificado. Envie ou atualize seu documento em <a class="underline" href="/creator/settings">Configuracoes</a> para liberar saques.
                </div>
            <?php endif; ?>
            <div class="mt-6 grid grid-cols-1 gap-4 md:grid-cols-2">
                ...
                <div class="rounded-2xl bg-[#f5f3f5] px-5 py-4 text-sm font-bold text-slate-600">
                    Metodo de saque
                    <p class="mt-2 text-base font-extrabold text-slate-900">PIX</p>
                </div>
                <input class="rounded-2xl border-none bg-[#f5f3f5] px-5 py-4 md:col-span-2" name="payout_key" placeholder="Chave PIX" required type="text" value="<?= e((string) ($payoutProfile['key'] ?? '')) ?>">
                <textarea class="min-h-[118px] rounded-2xl border-none bg-[#f5f3f5] px-5 py-4 md:col-span-2" name="note" placeholder="Observacao para o financeiro">Saque solicitado pelo painel do criador.</textarea>
            </div>
            <div class="mt-6 flex-1"></div>
            <button class="signature-glow w-full rounded-full px-6 py-4 text-sm font-bold text-white disabled:cursor-not-allowed disabled:opacity-60" <?= $canWithdraw ? '' : 'disabled' ?> data-prototype-skip="1" type="submit">Solicitar saque PIX</button>
        </form>
    </div>
    ...
    <section class="mt-6 rounded-3xl bg-white p-6 shadow-[0px_20px_40px_rgba(27,28,29,0.06)] lg:p-8">
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <h3 class="headline mt-2 text-2xl font-extrabold">Historico financeiro</h3>
                <p class="mt-3 max-w-2xl text-sm text-slate-500">Filtre entradas e saidas e acompanhe o status de cada movimentacao em tabela.</p>
            </div>
            <div class="rounded-2xl bg-[#f5f3f5] px-5 py-4 text-sm text-slate-600">
                ...
                <span class="ml-2 font-semibold">lancamentos</span>
            </div>
        </div>
        ...
        <form action="/creator/wallet" class="mt-6 grid grid-cols-1 gap-4 rounded-3xl bg-[#f7f4f7] p-5 md:grid-cols-[minmax(0,1fr)_minmax(0,0.5fr)_auto_auto]" method="get">
            <input class="rounded-2xl border-none bg-white px-5 py-4" name="q" placeholder="Buscar transacao..." type="search" value="<?= e((string) ($filters['q'] ?? '')) ?>">
        ...
        <div class="mt-6 overflow-x-auto">
            <table class="min-w-full divide-y divide-[#f0e8ee]">
                ...
                <?php foreach ($transactions as $transaction): ?>
                    ...
                    $transactionStatus = (string) ($transaction['status'] ?? ($type === 'payout' ? 'pending' : 'completed'));
                    $transactionStatusLabel = match ($transactionStatus) {
                        'paid', 'completed' => 'Concluido',
                        'processing' => 'Em processamento',
                        'rejected' => 'Rejeitado',
                        'pending' => 'Pendente',
                        default => ucfirst($transactionStatus),
                    };
                    $transactionStatusClass = match ($transactionStatus) {
                    ...
                    <tr class="align-top">
                        <td class="px-4 py-4">
                            <p class="font-bold text-slate-900"><?= e((string) ($transaction['note'] ?? ($typeLabels[$type] ?? 'Movimentacao'))) ?></p>
                            <?php if (trim((string) ($transaction['admin_note'] ?? '')) !== ''): ?>
                                <p class="mt-1 text-sm text-slate-500"><?= e((string) ($transaction['admin_note'] ?? '')) ?></p>
                            <?php endif; ?>
                        </td>
                        <td class="px-4 py-4">
                            <span class="rounded-full bg-[#f7f4f7] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.22em] text-slate-600"><?= e($typeLabels[$type] ?? 'Movimentacao') ?></span>
                        </td>
                        <td class="px-4 py-4">
                            <span class="inline-flex rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.22em] <?= e($transactionStatusClass) ?>"><?= e($transactionStatusLabel) ?></span>
                        </td>
                ...
                <?php if ($transactions === []): ?>
                    <tr>
                        <td class="px-4 py-8 text-sm text-slate-500" colspan="5">Nenhuma transacao encontrada com esse filtro.</td>
                    </tr>
                <?php endif; ?>
                </tbody>
            </table>
